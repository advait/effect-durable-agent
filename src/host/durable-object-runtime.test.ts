...
import { CommandId, EventId, SequenceNumber, SessionId } from "../types/core";
import {
...
import { CompactionExecutor, CompactionPolicy } from "../services/compaction";
import { frameworkReducedStateReducerName } from "../domain/reduced-state";
import type { EDAReducer } from "../services/reducer-registry";
import type { DurableObjectSqlCursor } from "./durable-object-storage";
import { makeLanguageModelLayer, type TestModelParts } from "../testkit/layers";
import {
...
import { makeRootEDATraceMetadata } from "../types/tracing";
...
describe("EDASessionDurableObjectHost", () => {
  ...
  it("closes restored WebSockets with malformed EDA attachments", async () => {
    const storage = new FakeDurableObjectStorage();
    await Effect.runPromise(EDASessionDurableObjectHost.migrate(storage));
    const host = makeHost(storage);
    const webSocket = new TestWebSocket();
    ...
    await host.restoreEventWebSockets([webSocket.asWebSocket()]);

    expect(webSocket.closeCode).toBe(EDA_WS_CLOSE_PROTOCOL_ERROR);
    expect(webSocket.closeReason).toBe("attachment");
  });
...
const collectWebSocketEventsUntil = async (
  ...
  predicate: (event: { readonly event: { readonly type: string } }) => boolean,
) => {
  const events: Array<{
