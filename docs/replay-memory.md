...
Startup changes no durable schema or checkpoint format. Framework and app reducers begin at their existing checkpoint sequences. A sink at exactly the hydrated session sequence shares that immutable framework/app projection. A lagging sink reconstructs its own projections from genesis through its cursor, then delivers only later events. Reducers must return new state rather than mutate shared input. The exported `EDASinkRunnerStartInput` now takes `initialProjection` instead of `initialHead`, deriving the head from `initialProjection.reduced.lastSeq`; direct callers must pass framework and app state at that same sequence. Persisted sink cursors and payloads are unchanged. Every sink inbox is registered before independent delivery fibers start, preventing an early sink's staged append from bypassing a later sink during startup. Lagging workers reconstruct lazily at delivery time, with reconstruction serialized across sinks to bound concurrent replay memory. Caught-up workers can deliver while another worker reconstructs. Durable notifications coalesce to the latest head; ephemeral inboxes are bounded by event count and encoded bytes.
